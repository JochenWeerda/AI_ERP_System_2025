...
Write-Host "1. Anwenden der Dashboard Styles ConfigMap..." -ForegroundColor Yellow
kubectl apply -f dashboard-styles.yaml

Write-Host "2. Anwenden der Dashboard ConfigMap..." -ForegroundColor Yellow
kubectl apply -f dashboard-part1.yaml

Write-Host "3. Anwenden des Dashboard Deployments..." -ForegroundColor Yellow
kubectl apply -f dashboard-part2.yaml

Write-Host "4. Anwenden der Dashboard Services..." -ForegroundColor Yellow
kubectl apply -f dashboard-part3.yaml

Write-Host "5. Warten auf Bereitschaft des Pods..." -ForegroundColor Yellow
kubectl wait --namespace erp-system --for=condition=ready pod --selector=app=valeo-dashboard --timeout=90s

Write-Host "6. Dashboard-Status:" -ForegroundColor Green
...
Write-Host "Alternativ: http://localhost:30090" -ForegroundColor Cyan
