...
export default defineConfig({
  ...
  server: {
    ...
    proxy: {
      ...
      // Analytics-Endpunkt für Web Vitals
      '/api/analytics': {
        target: 'http://localhost:8003',
        changeOrigin: true,
        rewrite: (path) => path
      },
      // Health-Check-Endpunkt für Web Vitals Service
      '/health': {
        target: 'http://localhost:3000',
        changeOrigin: true,
        rewrite: (path) => path
      },
      // Direkte Proxys zu Web-Vitals-Service
      '/api/collect/web-vitals': {
        target: 'http://localhost:3000',
        changeOrigin: true,
        rewrite: (path) => path
      }
  ...
  // Umgebungsvariablen
  define: {
    // Standardwerte für Umgebungsvariablen setzen
    'import.meta.env.VITE_ANALYTICS_ENDPOINT': JSON.stringify('/api/collect/web-vitals')
  }
}); 
